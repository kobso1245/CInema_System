...
def execute_count_projections_query_and_get_result(proj_id):
    cursor = CONN.cursor()
    data = cursor.execute(
        COUNT_FOR_PROJECTION_QUERY.format(proj_id)).fetchone()
    return data['count']
...
def show_ask_and_return_answer_for_projection_seats(
        movie_id, tickets, proj_id):
...
def execute_show_all_taken_seats_query(proj_id):
    cursor = CONN.cursor()
    data = cursor.execute(
        SHOW_ALL_TAKEN_SEATS_QUERY.format(proj_id)).fetchall()
...
def get_available_seats(proj_id):
    ...
    all_seats = [(row, col) for row in range(1, ROWS + 1)
...
def on_finalize(seats, proj_id, name):
    ...
    for seat in seats:
        cursor.execute(
            INSERT_INTO_RESERVATIONS.format(
                to_string(name),
                proj_id,
                seat[0],
                seat[1]))
...
def make_reservation():
    ...
    has_available_spots = show_ask_and_return_answer_for_projection_seats(
        movie_id, tickets, proj_id)
